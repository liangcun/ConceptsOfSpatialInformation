 ...
 * The implementation has migrated from ArcGIS JavaScript API 3.x to 4.4
 * The constructor was refactored to support "overloading".
 * version: 2.0.0
 * (c) Liangcun Jiang
 * latest change: Sep 2, 2019.
 */
define([
    "dojo/_base/declare",
    ...
    "dojo/domReady!"
], function (declare, ImageryLayer, RasterFunction, DimensionalDefinition, MosaicRule, Point, ImageServiceIdentifyTask, ImageServiceIdentifyParameters) {
    ...
    return declare(null, {
        ...
        /**
         *Field function: Gets field's value at a specific position
         *@param x: x coordinate of a point position
         *@param y: y coordinate of a point position
         */
        valueAt: function (x, y) {
            var identifyTask = new ImageServiceIdentifyTask(this.layer.url);
            var params = new ImageServiceIdentifyParameters();
            var sr = this.layer.spatialReference;
            var p = new Point(x, y, sr);
            params.geometry = p;
            params.returnCatalogItems = false;
            return identifyTask.execute(params).then(function (response) {
